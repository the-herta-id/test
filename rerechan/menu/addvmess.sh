...
if [ -n "$masaaktif" ]; then
    exp=$(((masaaktif + 1) * 24 * 60 * 60))
    exp2=$(date -d "$((masaaktif + 1)) days" +"%Y-%m-%d")
...
MOHON MELAKUKAN PERPANJANGAN VPN MAKSIMAL 3 HARI SEBELUM TANGGAL EXPIRED SETIAP BULAN NYA!
...
case "${reset_strategy}" in
  "no_reset") reset_strategy="Tidak Ada Reset";;
...
echo -e "e.) HTTP Upgrade TLS & NonTLS: /vmess-http"
echo -e "================================="
echo -e "alpn: "
